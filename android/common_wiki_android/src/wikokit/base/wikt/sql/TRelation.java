/* TRelation.java - SQL operations with the table 'relation' in SQLite Android 
...
public class TRelation {
    ...
    public static Map<String, List<String>> getAllWordPairs (SQLiteDatabase db) {
        ...
        float   t_work;
/*
        int n_total = Statistics.Count(SQLiteDatabase db, "relation");
     ...
     */
    private static Relation getRelationBetweenPageTitleAndWord (SQLiteDatabase db,String page_title,String word) {
        
        if(0 == word.length() || 0 == page_title.length() || word.equals(page_title))
            return null;

        TPage page = TPage.get(db, page_title);    if(null == page)        return null;
        TPage page_word = TPage.get(db, word);     if(null == page_word)   return null;
            
        TLangPOS[] lp = TLangPOS.get(db, page);
        for(TLangPOS lang_pos : lp) {
            TMeaning[] tm = TMeaning.get(db, lang_pos);
            for(TMeaning tmeaning : tm) {
                TRelation[] tr = TRelation.get(db, tmeaning);
                for(TRelation trelation : tr) {
                    TPage p = TRelation.getWikifiedPage(db, trelation);
    ...
    public static Relation getRelationType (SQLiteDatabase db,String word1,String word2) {
        ...
        r = TRelation.getRelationBetweenPageTitleAndWord(db, word2, word1);
        return r;
    }
}
